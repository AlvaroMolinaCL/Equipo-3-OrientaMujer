    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Editar permisos para {{ $tenant->name }}</>
            </h2>
                    ...
                    <form method="POST" action="{{ route('tenants.permissions.update', $tenant) }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach ($permisos as $permiso)
                                <div
                                    class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm hover:bg-gray-100 dark:hover:bg-gray-600 transition duration-200 ease-in-out">
                                    <input type="checkbox" name="permisos[]" value="{{ $permiso }}"
                                        id="permiso-{{ $permiso }}"
                                        class="mr-3 h-5 w-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
